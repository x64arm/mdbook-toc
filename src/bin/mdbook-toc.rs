...
fn build_toc(toc: &[(u32, String, String)]) -> String {
    ...
    // Generate ordered list numbers per level, resetting within each parent
    let mut level_counts: Vec<usize> = Vec::new();
    // CommonMark 对嵌套有序列表更稳定的缩进是每层 4 空格
    let indent_per_level = 4;

    for (level, name, slug) in toc {
        let idx = (level - min_level) as usize;

        // Ensure counters length fits current level; truncate deeper levels when moving up
        if level_counts.len() <= idx {
            level_counts.resize(idx + 1, 0);
        } else {
            level_counts.truncate(idx + 1);
        }

        // Increment current level counter
        level_counts[idx] += 1;

        // Indentation based on level depth
        let width = indent_per_level * idx;

        // Build hierarchical numbering like 1.1.2 from the current path
        let numbering = level_counts
            .iter()
            .map(|n| n.to_string())
            .collect::<Vec<_>>()
            .join(".");

        // Use HTML non-breaking spaces and <br/> to ensure visual indentation and line breaks
        let nbsp = "&nbsp;".repeat(width);
        writeln!(
            result,
            "{}{}. <a href=\"#{}\">{}</a><br/>",
            nbsp,
